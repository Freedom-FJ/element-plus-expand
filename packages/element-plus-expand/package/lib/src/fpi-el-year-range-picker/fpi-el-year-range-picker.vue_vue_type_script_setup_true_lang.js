  ...
  setup(__props, { emit: emits }) {
    ...
    const selectStartKey = vue.computed(() => {
      if (selectStart.value) {
        return dayjs__default.default(selectStart.value).year();
      }
      return 0;
    });
    ...
    const selectEndKey = vue.computed(() => {
      if (selectEnd.value) {
        return dayjs__default.default(selectEnd.value).year();
      }
      return 0;
    });
    ...
    const displayLeftValue = vue.computed({
      get: () => {
        ...
        return dayjs__default.default(props.modelValue[0]).format(props.showFormat);
      },
      set: () => {
      }
    });
    const displayRightValue = vue.computed({
      get: () => {
        ...
        return dayjs__default.default(props.modelValue[1]).format(props.showFormat);
      },
      set: () => {
      }
    });
    ...
    const popperYearList = vue.computed(() => {
      ...
      if (popperEndYear.value) {
        for (let index = showYearNum - 1; index >= 0; index--) {
          ...
          list.push({
            text: dayjs__default.default(`${year}`).format(props.cellFormat),
            year,
            yearStart: dayjs__default.default(`${year}`).startOf("year").toDate(),
            yearEnd: dayjs__default.default(`${year}`).endOf("year").toDate(),
            disabled
          });
        }
      }
    ...
    const handleOpenPopper = () => {
      ...
      if (props.modelValue[1]) {
        ...
        popperEndYear.value = dayjs__default.default(props.modelValue[1]).year();
      } else {
