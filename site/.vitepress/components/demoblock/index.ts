...
export default (md: any) => {
    ...
    md.render = (...args: any[]) => {
        ...
        demoLabels?.forEach(async (demo: any) => {
            ...
            const showPart = getDemoLabel(demo, 'showPart') || undefined // demo lang
            ...
            if (showPart && codeStr !== 'src文件不存在') {
                const scriptStart = codeStr.slice(codeStr.indexOf(`<${showPart}`))
                codeStr = scriptStart.slice(scriptStart.indexOf('>') + 1, scriptStart.indexOf(`</${showPart}>`))
            }
            htmlStr = codeToHtml(codeStr, { lang: showPart ? showPartToLang[showPart] : demoLang })// demo的html字符串
