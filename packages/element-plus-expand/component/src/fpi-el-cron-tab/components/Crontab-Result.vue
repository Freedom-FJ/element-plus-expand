...
export default defineComponent({
    ...
    data() {
        return {
            dayRule: '',
            dayRuleSup: '' as string | number | number[],
            dateArr: [] as number[][],
    ...
    methods: {
        ...
        expressionChange() {
            ...
            // 将获取到的数组赋值-方便使用
            const sDate: number[] = this.dateArr[0]
            const mDate: number[] = this.dateArr[1]
            const hDate: number[] = this.dateArr[2]
            const DDate: number[] = this.dateArr[3]
            const MDate: number[] = this.dateArr[4]
            const YDate: number[] = this.dateArr[5]
            // 获取当前时间在数组中的索引
            let sIdx = this.getIndex(sDate, nSecond) as number
            let mIdx = this.getIndex(mDate, nMin) as number
            let hIdx = this.getIndex(hDate, nHour) as number
            let DIdx = this.getIndex(DDate, nDay) as number
            let MIdx = this.getIndex(MDate, nMouth) as number
            const YIdx = this.getIndex(YDate, nYear) as number
            // 重置月日时分秒的函数(后面用的比较多)
            ...
            // 如果当前年份不为数组中当前值
            if (nYear !== YDate[YIdx as number]) {
                resetMouth()
            }
            // 如果当前月份不为数组中当前值
            if (nMouth !== MDate[MIdx as number]) {
                resetDay()
            }
            // 如果当前“日”不为数组中当前值
            if (nDay !== DDate[DIdx as number]) {
                resetHour()
            }
            // 如果当前“时”不为数组中当前值
            if (nHour !== hDate[hIdx as number]) {
                resetMin()
            }
            // 如果当前“分”不为数组中当前值
            if (nMin !== mDate[mIdx as number]) {
                resetSecond()
            }

            // 循环年份数组
            goYear: for (let Yi = YIdx; Yi as number < YDate.length; Yi++) {
                const YY = YDate[Yi as number]
                // 如果到达最大值时
                ...
                goMouth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
                    ...
                    goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
                        ...
                        if (this.dayRule === 'lastDay') {
                            ...
                            if ((this.dayRuleSup as number[]).includes(Number(thisWeek))) {
                            ...
                            // 如果指定了是第几周的星期几
                            // 获取每月1号是属于星期几
                            const thisWeek = Number(this.formatDate(new Date(`${YY}-${MM}-${DD} 00:00:00`), 'week'))
                            if ((this.dayRuleSup as number[])[1] >= thisWeek) {
                                DD = ((this.dayRuleSup as number[])[0] - 1) * 7 + (this.dayRuleSup as number[])[1] - thisWeek + 1
                            }
                            else {
                                DD = (this.dayRuleSup as number[])[0] * 7 + (this.dayRuleSup as number[])[1] - thisWeek + 1
                            }
                        }
                            ...
                            // 获取月末最后一天是星期几
                            const thisWeek = this.formatDate(new Date(`${YY}-${MM}-${thisDD} 00:00:00`), 'week') || 0
                            // 找到要求中最近的那个星期几
                            if (this.dayRuleSup < thisWeek) {
                                DD -= (thisWeek as number) - (this.dayRuleSup as number)
                            }
                            else if (this.dayRuleSup > thisWeek) {
                                DD -= 7 - ((this.dayRuleSup as number) - Number(thisWeek))
                            }
                        }
        ...
        // 用于计算某位数字在数组中的索引
        getIndex(arr: number[], value: number) {
        ...
        // 根据规则返回一个具有周期性的数组
        getCycleArr(rule: string, limit: number, status: boolean) {
        ...
        formatDate(value: number | Date, type?: string) {
            // 计算日期相关值
        ...
        // 检查日期是否存在
        checkDate(value: number | Date | string) {
